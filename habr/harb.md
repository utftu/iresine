...
iresine это библиотека, которая создана для нормализации данных и оповещении об их изменении.

В данный момент iresine состоит из следующих модулей:
* @iresine/core - основной модуль обрабатывающий данные.
* @iresine/react-query - плагин для react-query. Позволяет выполнять действия по нормализации напрямую с данными в хранилище react-query. Вся работа происходит автоматически и клиент работает с react-query так, как бы работал без iresine.
