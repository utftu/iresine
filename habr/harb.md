...
И если пример с mobx выглядит приемлемо, то нормализация в redux просто [ужасает](https://redux.js.org/recipes/structuring-reducers/normalizing-state-shape). Работать с таким кодом становится сложнее по мере его увеличения и совсем неинтересно
...
Apollo достигает указанного эффекта запрашивая при каждом запросе внутреннее поле \_\_typename,
...
возможностью автоматического отслеживания изменения в данных. Именно этот вариант я выбрал как предпочтительным для себя.
...
user имеет своей идентификатор в хранилище, а как быть с jobs? У них нет ни поля type ни поля field!
@iresine/core следует простому правилу: если у сущности нет идентификатора, то она становится частью ближайшей
родительской сущности с идентификатором.

@iresine/core являет универсальной библиотекой, которая знает о том как распарсить данные и точечно уведомлять подписчиков,
но использовать ее напрямую довольно нудно и утомительно, посмотрим как сделать этот процесс удобнее!

## @iresine/react-query

react-query это прекрасная библиотека, с которой каждому я бы советовал ознакомиться.
Количество фич кажется запредельным: от кэширования запросов до синхронизации данных между окнами браузера.
Но в ней отсутствует нормализация данных и именно этот факт вдохновил меня на написания iresine.

@iresine/react-query это плагин для react-query, он позволяет использовать функцию нормализации и обновления данных @iresine/core на данных хранилища react-query.
Вся работа по нормализации происходит автоматически и клиент работает с react-query так, как бы работал без iresine.
...
Нормализация данных на клиенте это проблема. Эта проблема решается разным способами с разной степенью успешности.
В написанном выше материале автор предлагает способ решения этой проблемы. Если сократить все предложение до нескольких слов,
то они будут звучать как **_добавьте информацию о типах в данные, а после этого используйте iresine_**
