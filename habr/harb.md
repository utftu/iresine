...
Это самые очевидный и заманчивый вариант. В некоторых случаях клиентское
приложение действительно может позволить себе иметь одинаковые сущности с
разными данными. Однако что делать со случаями когда это недопустимое поведение?
...
Однако такое решение заставляет использовать нас graphql и apollo, которые, по
моему мнению, имеют множеств недостатков
...
отслеживания изменения в данных. Именно этот вариант я выбрал как
...
поэтому рассмотрим вопрос в какой момент добавлять информацию о типах в данных
...
- @iresine/core
- @iresine/react-query - плагин для react-query. Он позволяет выполнять действия
  по нормализации напрямую с данными в хранилище react-query. Вся работа
  происходит автоматически и клиент работает с react-query так, как бы работал
  без iresine.
...
сделать использовать такой же идентификатор как в apollo:
...
Так же мы можем обрабатывать и глобально уникальное поле id, применит небольшой
хак:
...
react-query это прекрасная библиотека, с которой каждому я бы советовал
ознакомиться. Количество фич кажется запредельным: от кэширования запросов до
синхронизации данных между окнами браузера. Но в ней отсутствует нормализация
данных и именно этот факт вдохновил меня на написания iresine.
...
Нормализация данных на клиенте это проблема. Эта проблема решается разным
способами с разной степенью успешности. В написанном выше материале автор
предлагает способ решения этой проблемы. Если сократить все предложение до
