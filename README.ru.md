...
uresine.subscribe('user:0', () => {...}) // само собой iresine умеет подписывать за изменение в сузностях
...
Как быть с объектами у которых нет ни поля
...
идентификатором или в случае если ее отсутсвия не отслеживается вовсе.
...
А испольвание react-query остается неизменным
```js
...
Вот так выглдяит все работающий пример:
